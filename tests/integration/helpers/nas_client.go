...
// CreateTestDirectory creates a directory on the QNAP NAS for testing
func CreateTestDirectory(conn *qnap.Connection, path string) error {
...
// CreateTestFile creates a file with specified content on the QNAP NAS
func CreateTestFile(conn *qnap.Connection, filePath, content string) error {
...
func WaitForContainer(conn *qnap.Connection, containerName, expectedState string, timeout time.Duration) error {
...
func CheckContainerLogs(conn *qnap.Connection, containerName, expectedLogContent string) error {
...
func GetContainerIP(conn *qnap.Connection, containerName string) (string, error) {
...
func TestContainerConnectivity(conn *qnap.Connection, fromContainer, toContainer string, port int) error {
...
func WaitForContainerState(conn *qnap.Connection, containerName, expectedState string, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
...
func VerifyImageExists(conn *qnap.Connection, imageName string) (bool, error) {
...
func VerifyVolumeExists(conn *qnap.Connection, volumeName string) (bool, error) {
...
func VerifyNetworkExists(conn *qnap.Connection, networkName string) (bool, error) {
...
func CreateTestFileInContainer(conn *qnap.Connection, containerName, filePath, content string) error {
...
func ReadTestFileFromContainer(conn *qnap.Connection, containerName, filePath string) (string, error) {
	opts := &deploy.ExecOptions{
		Interactive: false,
		TTY:         false,
	}
