...
import (
	...
	"github.com/scttfrdmn/qnap-docker/tests/integration/helpers"
)

var (
	integrationTest = flag.Bool("integration", false, "Run integration tests")
	nasHost         = flag.String("nas-host", "", "QNAP NAS IP address or hostname")
...
func TestMain(m *testing.M) {
	...
	// Other tests like TestConnectionToAstrapi can run independently
...
func setupTestEnvironment() (*TestRunner, error) {
	...
	cfg := &config.Config{
		...
		Defaults: struct {
			...
			PrimaryVolume string `yaml:"primary_volume,omitempty"`
			Network       string `yaml:"network,omitempty"`
		}{
			VolumePath:    "/share/CACHEDEV1_DATA/qnap-docker-test",
			PrimaryVolume: "CACHEDEV1_DATA",
			Network:       "bridge",
		},
	}
	...
	if err := conn.TestConnection(); err != nil {
		return nil, fmt.Errorf("Container Station not available on NAS: %w", err)
	}
...
func testBasicDeployment(t *testing.T) {
	...
	htmlContent := fmt.Sprintf("<h1>qnap-docker Test - %s</h1>", time.Now().Format("2006-01-02 15:04:05"))
	...
	if err := helpers.TestHTTPEndpoint(fmt.Sprintf("http://%s:8080", testRunner.Config.Host),
		"qnap-docker Test", 30*time.Second); err != nil {
