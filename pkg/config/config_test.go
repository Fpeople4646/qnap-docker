...
func TestNew(t *testing.T) {
	...
	if config.Defaults.PrimaryVolume != "CACHEDEV1_DATA" {
		t.Errorf("Expected primary volume CACHEDEV1_DATA, got %s", config.Defaults.PrimaryVolume)
	}
}
...
func TestSaveAndLoad(t *testing.T) {
	...
	config := &Config{
		...
		Defaults: struct {
			...
			VolumePath:    "/share/CACHEDEV2_DATA/test",
			PrimaryVolume: "CACHEDEV2_DATA",
			Network:       "testnet",
		},
	}
	...
	if loadedConfig.Defaults.PrimaryVolume != config.Defaults.PrimaryVolume {
		t.Errorf("Expected primary volume %s, got %s", config.Defaults.PrimaryVolume, loadedConfig.Defaults.PrimaryVolume)
	}
