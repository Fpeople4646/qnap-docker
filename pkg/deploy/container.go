...
type ContainerOptions struct {
	...
	Volumes     []string // ["/share/CACHEDEV1_DATA/data:/app/data"]
...
func NewContainerOptions(image string) *ContainerOptions {
	return &ContainerOptions{
		Image:       image,
		Restart:     qnap.DefaultRestartPolicy,
		NetworkMode: qnap.DefaultNetwork,
	}
}

// Container deploys a container using direct Docker commands over SSH
func Container(conn *qnap.Connection, opts *ContainerOptions) (string, error) {
	if opts.Name == "" {
...
func ListContainers(conn *qnap.Connection, all bool) ([]ContainerInfo, error) {
...
func RemoveContainer(conn *qnap.Connection, nameOrID string, force bool) error {
	args := []string{"rm"}
	if force {
		args = append(args, "-f")
	}
...
func TestDockerConnection(conn *qnap.Connection) error {
	// Test Docker command
