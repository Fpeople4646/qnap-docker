...
// Compose deploys a docker-compose file to the QNAP NAS
func Compose(conn *qnap.Connection, opts *ComposeOptions) error {
...
func convertServiceToContainer(service ComposeService, containerName string, envVars map[string]string) (*ContainerOptions, error) {
	...
	if opts.Restart == "" {
		opts.Restart = qnap.DefaultRestartPolicy
	}
